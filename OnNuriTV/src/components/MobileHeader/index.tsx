    ...
    return (
        <>
            <div className='flex justify-between gap-6 p-4 bg-mobile-header'>
                <div onClick={handleBackNavigateHome} className='relative w-[100px] h-10 object-fill'>
            ...
            >
                ...
                <ul className='flex flex-col gap-8 py-2 text-white my-7'>
                    <div className={`relative w-full text-sm font-semibold transition-colors duration-300 cursor-pointer hover:text-yellow-500 ${currentPage === 0 ? 'text-[#5176FF]' : 'text-white'}`} onClick={() => handleNavigate()}>
                        ...
                        return (
                            <div key={detail.id} className='relative w-full'>
                                <div onClick={() => handleNavigate(detail)}
                                    className={`text-sm font-semibold transition-colors duration-300 cursor-pointer hover:text-yellow-500 ${currentPage === detail.id ? 'text-[#5176FF]' : 'text-white'}`}>
